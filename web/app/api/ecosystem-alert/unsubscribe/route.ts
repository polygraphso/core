/**
 * /api/ecosystem-alert/unsubscribe?token=<unsubscribe_token>
 *
 * One-click unsubscribe for an ecosystem digest recipient. GET is the human click
 * from the email (returns a small confirmation page); POST is the RFC 8058
 * one-click the List-Unsubscribe-Post header points at. Both flip unsubscribed_at
 * and are idempotent.
 *
 * Token-scoped, so no auth: the per-row random token is the capability. This lives
 * outside /api/manage deliberately — it's an unauthenticated capability endpoint.
 */
...
/** Flip the recipient off. Returns false only on a real DB error. */
async function unsubscribe(token: string): Promise<boolean> {
  const db = getSupabase();
  if (!db) {
    console.error("[ecosystem-unsubscribe] Supabase not configured");
    return false;
  }
  ...
  if (error) {
    console.error("[ecosystem-unsubscribe] update failed:", error.message);
    return false;
  }
  return true;
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  return page(
    "You're unsubscribed",
    "<p>You won't get any more weekly monitoring digests for this ecosystem. An admin can re-add you from the ecosystem's alert settings.</p>",
    200,
  );
}
