...
export function MonitorForm(props: Props) {
  ...
  async function submit(payload: { server_ref: string; email?: string; company?: string }) {
    ...
    try {
      ...
      setStatus("ok");
      setMessage(`You're monitoring ${serverRef}. We'll email you when a new version is graded.`);
    } catch (err) {
      setStatus("error");
      setMessage(
        err instanceof Error
          ? err.message
          : "Couldn't save your subscription. Try again or email hello@polygraph.so.",
      );
    }
  }
  ...
  if (mode === "signed-in") {
    return (
      <div className="grid gap-3">
        ...
        >
          <span>{status === "submitting" ? "Saving…" : "Monitor this server"}</span>
  ...
  return (
    ...
    >
      ...
      >
        <span>{status === "submitting" ? "Saving…" : "Monitor"}</span>
        <span aria-hidden className="text-base leading-none">→</span>
      </button>
      <p
        id="monitor-status"
