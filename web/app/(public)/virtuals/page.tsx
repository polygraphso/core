 ...
 * sitemap, robots noindex — reachable only by direct link. Reads grades live from
 * hosted_runs INCLUDING unpublished rows (see lib/virtualsIndex), so it shows grades
 * not yet public anywhere else. Keep it private until a publish decision is made per
 * server.
 */
export const metadata: Metadata = {
  title: "Virtuals-Protocol MCP index — polygraph (private)",
...
/** Category + a third-party flag (the trust caveat — a community wrapper, not the project's own). */
...
export default async function VirtualsIndexPage() {
    ...
    <article>
      <header className="mb-9">
        ...
        <p className="mt-5 font-serif italic text-ink-muted text-lg md:text-xl leading-snug max-w-2xl">
          ...
          the protocol&rsquo;s own commerce and framework infrastructure, and the agents and apps
          launched on it.
        </p>
      </header>
      ...
      {/* Thesis: the launchpad has no behavioral vetting; this is that vetting. */}
      <figure className="mb-9 border-l-2 pl-5" style={{ borderColor: "var(--color-oxblood)" }}>
        <blockquote className="font-serif text-ink text-lg md:text-xl leading-snug">
          Virtuals launches agents permissionlessly — the protocol doesn&rsquo;t vet how any of them
          <em> behave</em>. <span className="text-oxblood">This is that vetting</span>, applied to the
          ecosystem&rsquo;s MCP surface.
        </blockquote>
        <figcaption className="mt-3 text-[13px] leading-relaxed text-ink-muted max-w-2xl">
          Each server is graded on its own MCP surface. Most agents launch via GAME/ACP rather than a
          bespoke server, so they carry <strong className="text-ink-muted">no standalone MCP</strong> to
          grade — mapped honestly below. Grades reflect the testable surface (state-changing tools
          aren&rsquo;t exercised by default); remote-only servers cap at{" "}
          <strong className="text-ink-muted">B</strong> (egress unverifiable), npm servers run sandboxed
          and can reach A, and a C-04 (adversarial-input) failure caps a grade at D.{" "}
          <span className="text-oxblood/70">3rd-party</span> marks a community wrapper rather than the
          project&rsquo;s own server. These rows are <strong className="text-ink-muted">not published</strong>{" "}
