...
export const BANKR_AGENTS_META: BankrAgentMeta[] = [
  ...
  { project: "gitlawb", handle: "Gitlawb", mcpRef: "stdio · gl mcp serve", target: null, note: "ships a standard stdio MCP server (24 git/identity tools) behind a gl mcp serve subcommand. The harness can now target the subcommand, but the gl CLI is a precompiled static binary: the published @gitlawb/gl package carries an empty bin that an install-time step fills from per-platform @gitlawb/gl-* packages, which the sandbox does not run, so there is no runnable entry to launch" },
  { project: "Azzle", handle: "dabusthebuilder", mcpRef: "npm/@azzle/agents", target: null, note: "ships a stdio MCP server (~10 azzle_* tools) inside @azzle/agents at mcp/server.mjs, a subpath rather than the package's declared bin or main. The harness can now launch it by naming that entry file, and on evaluation it grades D (C-02 egress overreach: undeclared calls to api.studio.thegraph.com; C-01, C-03, and C-04 pass). The published grade is pending" },
  ...
  { project: "1Claw", handle: "1clawAI", mcpRef: "npm/@1claw/mcp", target: null, note: "publishes a real stdio MCP server for its secrets vault. The bin requires ONECLAW_* auth credentials at startup and exits before the MCP handshake without them. The harness can now inject those startup credentials, so grading becomes possible once a valid vault key is supplied; none is held today" },
