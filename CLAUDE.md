...
- `web/` — the polygraph.so site (Next.js 16, App Router, Tailwind v4): landing, the MCP Security Index (`/mcp-index`), per-server (`/mcp`) and per-skill (`/skill`) grade reports, ecosystem indices (`/bankr`, `/base`) and monitoring, the authenticated console (`/dashboard`, `/account`), self-serve checkout + `/pricing`, blog, and the embeddable grade badge. Has its own `AGENTS.md` warning that this Next.js may differ from training-data conventions; consult `web/node_modules/next/dist/docs/` before guessing.
...
Behavioral evaluation for MCP servers and the agents that use them. Free public grades + a CLI (`npx polygraphso check <server>`) that's a sub-second lookup over precomputed grades. Subscriber-pays only, priced and paid in $POLYGRAPH on Base via self-serve crypto checkout (no fiat except enterprise invoices). Independence is disclosure-based — material support is allowed if publicly registered, never undisclosed; no graded party gets review or approval rights. Live paid surfaces: priority grading (one-time), Pro monitor plans, and ecosystem monitoring subscriptions; enterprise vendor-risk scoring is the forward tier. Independence is the moat: frontier labs are too conflicted to grade their own ecosystem, Trustpilot-style ratings are too gameable.
