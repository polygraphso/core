...
export const CHECK_TOOL_DESCRIPTION = [
  ...
  "by evidence anyone can re-run. A polygraph pairs that grade with an",
  "adoption tier (Top 10 / 25 / 50 / 100).",
  "",
  ...
  "",
  "Returns one of:",
  "  - tracked: the server is in polygraph's set. `adoption_tier` is the",
  "    popularity signal. `polygraph` is the published grade ('A'|'B'|'D'|'F',",
  "    no C) or null if not graded yet; when present, `polygraph_detail` carries",
  "    the per-check results (C-01/C-02/C-03), tool-surface fingerprint, and",
  "    methodology version. The notify URL lets a user subscribe to updates.",
